...
namespace TaskvsThread
{
    public class CarBuilding
    {
        ...
        virtual protected Engine BuildEngine(int horsePower) =>
            new Engine(horsePower);

        virtual protected Suspension BuildSuspension(int supportedKg) =>
            new Suspension(supportedKg);

        virtual protected Painting Paint(string color, int bodyArea) =>
            new Painting(color, bodyArea);
