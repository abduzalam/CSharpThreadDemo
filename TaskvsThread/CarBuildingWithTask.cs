...
namespace TaskvsThread
{
    public class CarBuildingWithTask : CarBuilding
    {
        public static void Build(int horsePower, int suspensionCount)
        {
            ...
            var suspensionTasks = Enumerable.Range(0, suspensionCount)
                                            .Select(i => Task.Run(() => carBuilding.BuildSuspension(40)));

            var suspensionTask = Task.WhenAll(suspensionTasks).ContinueWith(task=>task.Result.ToList());

            try
            {
                var testingTask = Task.Run(()=> carBuilding.Test(bodyTask.Result, suspensionTask.Result,engineTask.Result));
